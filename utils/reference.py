...
class Reference:
    ...
    def _get_authors(self) -> int:
        self.authors = list(
            filter(
                ...
                map(self._get_author, self.data['author']),
            )
        )
        self.initial_authors = list(
            filter(
                lambda s: s is not None,
                map(self._get_initial_author, self.data['author']),
            )
        )
        return len(self.authors)

    def _get_journal(self) -> bool:
        self.full_journal, self._full_journal = self._get_params(
            'container-title'
        )
        self.short_journal, self._short_journal = self._get_params(
    ...
    def _get_year_old(self) -> bool:
        if 'license' not in self.data:
            ...
            return True
        years = list(
            map(lambda x: x['start']['date-parts'][0][0], self.data['license'])
        )
        self.year = years[0]
